WITH cm AS (
  ...
  /* Add a distinct to filter results before processing*/
      ...
      /* Normalized Source */
      CASE
        WHEN source IN ('Google Adwords','Ad Extension','Google Call Extension','call only','Google Ads') THEN 'Google Ads'
        ...
        WHEN source IN ('GMB','GMB ','GMB - Glen Ellyn, IL 60137','Google My Business','GMB Post',
                        ...
                        'GMB - Newport News','GMB - Glen Ellyn, IL 60137') THEN 'GBP'
        ...
        WHEN source IN ('bing', 'BING Paid','Bing Organic','Bing Call Extensions') THEN 'Microsoft Ads'
      ...
      CASE
        ...
        WHEN source IN ('GMB','GMB ','GMB - Glen Ellyn, IL 60137','Google My Business','GMB Post',
                        'GMB - Gurnee, IL 60031','North Chicago GMB','GMB - Brownsville',
                        'GMB - Newport News','GMB - Glen Ellyn, IL 60137') THEN 'Non-Paid'
        WHEN source IN ('facebook paid','General Meta Ads') THEN 'Paid'
        WHEN source IN ('Facebook video','Facebook Ads','facebook') THEN 'Paid'
        ...
        WHEN source IN ('Bing Paid','Bing Organic','Bing Call Extensions', 'Bing', 'Local Biz Emails', 'LocalBizCslls') THEN 'Paid'
...
subs AS (
  SELECT DISTINCT
      ...
      s.initialStatus,
      o.branchname,
      DATE(s.dateadded) AS activity_date
  FROM cm
...
FROM (
  ...
  SELECT
      Source,
      DATE(datecontacted) AS activity_date,
      1 AS total_leads,
