...
with aggregate19 as (
   select 
      ...
      sum(stayed1Y.contractValue) - sum(stayed1Y.initialservicetotal) as CVAfterY1,
      count(stayed2Y.subscriptionID) as SubsAfterY2, -- Year 2021
      sum(stayed2Y.contractValue) - sum(stayed2Y.initialservicetotal) as CVAfterY2,
      count(stayed3Y.subscriptionID) as SubsAfterY3, -- Year 2022
      sum(stayed3Y.contractValue) - sum(stayed3Y.initialservicetotal) as CVAfterY3,
      count(stayed4Y.subscriptionID) as SubsAfterY4, -- Year 2023 
      sum(stayed4Y.contractValue) - sum(stayed4Y.initialservicetotal) as CVAfterY4,
      count(stayed5Y.subscriptionID) as SubsAfterY5, -- Year 2024
      sum(stayed5Y.contractValue) - sum(stayed5Y.initialservicetotal) as CVAfterY5
   from ( -- Retrieves customers who started in a specific year and cancelled in a specific year. Also, joins subscriptions to CTM phone calls, which SHOULD eliminate those that were marked incorrectly as Internet source in PestRoutes
   	select s.subscriptionID, s.dateAdded, s.dateCancelled, s.contractValue, s.initialservicetotal 
   ...
   select 
   	2023 as Year,
      5548772.57 as Spend,
      21584 as SubsOrigin, # Year 2023
      16614070 as CVOrigin,
   ...
   select 
   	2024 as Year,
      4561323 as Spend,
      22592 as SubsOrigin, # Year 2024
      18337728 as CVOrigin,
